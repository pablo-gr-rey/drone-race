...
)
{
    ...
    for (int t = 0; t < T && !stop; t++)
    {
        ...
        TerminalType term = environmentStep<false, false>(t, theta, envConfig, egoAction, true, state, branchState, branched, 0.0f, buffer, drng,
                                                          agent, mppiConfig.gateTraversalMargin);
                                    ...
                                    int N, int P, float safetyWeight)
{
    ...
    cost[out] = avg + ((maxSafe > 0.0f) ? safetyWeight : 0.0f);
    cost[out + 1] = maxSafe;
}
...
)
{
    ...
    const float* cost_arr = which == 0 ? cost_nom : (which == 1 ? cost_rob : cost_rob + 1);
    float* cand_nominal = (which == 0) ? cand1_nominal : (which == 1 ? cand2_nominal : new_rob_nominal);
...
)
{
    ...
    for (int t = 0; t < mppiConfig.nTimesteps && !stop; ++t)
    {
        ...
        TerminalType term = environmentStep<false, false>(t, thetas[p], envConfig, egoAction, true, state, branchState, branched, 0.0f, buffer, drng,
                                                          agent, mppiConfig.gateTraversalMargin);
