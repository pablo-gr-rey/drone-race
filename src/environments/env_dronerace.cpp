...
namespace EnvDroneRace
{
    ...
    std::pair<EnvironmentConfig, int> unpackEnvConfig(const void* buf, size_t len)      // return seed
    {
        ...
        std::cout << "loaded envConfig, nAgents " << nAgents << " nWinLaps " << envConfig.nWinLaps << " true theta " << envConfig.trueTheta << " max speed " << envConfig.maxSpeed[0] << ' ' << envConfig.maxSpeed[1] << " nTrackSamples" << nTrackSamples << " gate vectors:";
