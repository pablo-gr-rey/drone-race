...
HD INLINE int findConfident(const float* __restrict__ belief, float threshold)
{
    for (int theta = 0; theta < N_MODELS; theta++)
...
{
    for (int k = 0; k < N_MODELS; k++)
        branchState.belief[k] = initBelief[k];

    branchState.predTheta = findConfident(branchState.belief, threshold);
    branchState.branchingTime = 0;
}
