...
namespace EnvDroneRace
{
    ...
    // Boundary distance = distance of point to closest boundary or opponent (<= 0 if outside), does not take into account agent's radius. return -1.0f if other agent won
    HD INLINE float trackBoundaryDist(const SimState& state, const EnvironmentConfig& envConfig)
    {
        ...
        for (int iObs = 0; iObs < N_OBSTACLES; iObs++)
        {
            ...
            for (int d = 0; d < DIM; d++)
            {
                float dist = fmaxf(0.0f, fmaxf(envConfig.obstacles[iObs * 2 * DIM + d] - curPos[d], curPos[d] - envConfig.obstacles[(iObs * 2 + 1) * DIM + d]));
                sqDist += dist * dist;
            }
    ...
    HD INLINE bool isOutside(const SimState& state, const EnvironmentConfig& envConfig, float radius)   // radius should be e.g. minDist/2 in the actual dynamics and (minDist * factor) / 2 for MPPI
    {
        // for (int d = 0; d < DIM; d++)
        ...
        float gateMargin = 0.0f)
    {
            ...
            else
            {
                for (int thetaT = 0; thetaT < N_TRUE_MODELS; thetaT++)
                    computePIDAction(
                        ...
                        scratch.nomPidActions + thetaT * DIM);
        ...
        if constexpr (shouldUpdateBelief)
            updateBelief(
                branchState.belief,
                scratch.actions + oppAgent * DIM,
                scratch.nomPidActions,
                envConfig.oppPid,
