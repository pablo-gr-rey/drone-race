...
class MPPIController : public Controller
{
  public:
    ...
    MPPIController(const EnvironmentConfig& c, const MPPIConfig& mc, int s, std::optional<std::vector<float>> nominal = std::nullopt);
    ...
    std::vector<float> buildSplineMatrix(); // return a T*M matrix B such that for u spline control points, B*u computes the (natural) spline for
                                            // each value of 0 <= t < T (in particular, if tau_i is integer, (Bu)_(tau_i) = u_i)
  ...
  private:
    ...
    // d_costs[(nom0/theta0, nom1/theta1, ...), s] = cost of sample s, averaged over unknown parameters (i.e. d_costs[(nom0,
    // theta1), s] = cost of sample s under theta1, averaged over theta0)
    ...
    // d_branchUsed[trueTheta, s, k] = 0 if sample s stayed in nominal for parameter k in case of trueTheta (and corresponding
    // branchTime = -1), otherwise 1 + value of model it switched to. if we are already committed at time 0, then equal to 1 +
    // theta (and corresponding branchTime = 0)
    int* d_branchUsed = nullptr; // (nTrueModels, N, nModelFactors)

    // d_branchTime[trueTheta, s, k] = branching time for parameter k in case of trueTheta (or -1 if no branching, 0 if we are
    // already branched at time 0). note that this is ABSOLUTE time, and that it corresponds to the first time in the new branch
    // (i.e. if we become certain on the first step, then branchingTime=1)
...
class PRMPPIController : public Controller
{
  public:
    ...
    PRMPPIController(const EnvironmentConfig& c, const PRMPPIConfig& mc, int s, std::optional<std::vector<float>> nominal = std::nullopt);
    ...
    bool resetNom;   // true if the new nominal is optimized from rob_nominal (happens if the nominal has a very high safety cost)
                     // (i.e. we use cand2 for new nom_nominal)
